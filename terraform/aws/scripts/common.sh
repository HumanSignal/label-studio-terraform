...
if [ -z "${VARS_FILE:-}" ]; then
  echo "[ERROR] This step requires to export the following variables VARS_FILE"
  exit 1
else
  if [ -f "${VARS_FILE}" ]; then
    echo -e "[INFO] Using '${VARS_FILE}' as a TF vars file"
  else
    echo "[ERROR] TF vars file '${VARS_FILE}' could no be found"
  fi
fi

export TF_PARAMS="-var-file=${VARS_FILE} ${ADDITIONAL_TF_PARAMS:-}"
export TF_VAR_environment=$(sed -n 's#^[ ]*environment[ ]*=[ ]*"\(.*\)"#\1#p' "${VARS_FILE}")
export TF_VAR_name=$(sed -n 's#^[ ]*name[ ]*=[ ]*"\(.*\)"#\1#p' "${VARS_FILE}")
export TF_VAR_region=$(sed -n 's#^[ ]*region[ ]*=[ ]*"\(.*\)"#\1#p' "${VARS_FILE}")
